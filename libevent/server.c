...
int main(void) {
  ...
  struct epoll_event events
      [MAX_EVENTS]; // epoll_wait的传出参数(数组：保存就绪事件的文件描述符)
  while (1) {
    ...
    int n_ready =
        epoll_wait(ep_fd, events, MAX_EVENTS, 1000); // 1秒没事件满足则返回0
    if (n_ready < 0) {
...
void initlistensocket(int ep_fd, unsigned short port) {
  ...
  /*绑定*/
  int ret = bind(listen_fd, (struct sockaddr *)&listen_socket_addr,
...
void acceptconnect(int listen_fd, int event, void *arg) {
  ...
  if ((connect_fd = accept(listen_fd, (struct sockaddr *)&connect_socket_addr,
                           &connect_socket_len)) < 0) {
  ...
  do {
    for (
        i = 0; i < MAX_EVENTS;
        i++) // 从全局数组ep_events中找一个空闲位置i(类似于select中找值为-1的位置)
...
void recvdata(int client_fd, int event, void *arg) {
  ...
  if (len > 0) {
    ...
    eventset(ev, client_fd, senddata,
             ev); // 2.设置client_fd对应的回调函数为senddata
...
void senddata(int client_fd, int event, void *arg) {
  ...
  if (len > 0) {
    ...
    eventset(ev, client_fd, recvdata,
             ev); // 2.设置client_fd对应的回调函数为recvdata
